...
if __name__ == '__main__':
    try:
        pub = rospy.Publisher('/bluerov2/thruster_manager/input', Wrench,
                              queue_size=10)  # Start Publisher publishing Wrench msgs on topic named: /bluerov2/thruster_manager/input.
        rospy.init_node('input_generator',
                        anonymous=True)  # Initialize the ROS node to allow it to communicate with the ROS Master (node name = input_generator).
        # anonymous = True ensures that your node has a unique name by adding random numbers to the end of NAME.
        ## Parameters to be changed:
        rate = rospy.Rate(50)  # Sample time in Hz, here 5 Hz.
        ...
        while not rospy.is_shutdown():  # Do while ROS is running.
            ...
            if elapsed_time > dur:  # stop producing values after duration.
                ...
                msg.force.z = float(0)  # Fill in the value at the desired DOF (Here heave (msg.force.z)).
                pub.publish(msg)  # Publish the msg to the ROS Master.
                break  # Break the program and kills the node.

            if elapsed_time > sum(t[:(period + 1)]):  # Check if elapsed_time exceeds the current time period.
                period += 1
            thrust = a[period]
...
# a = [6.0, 10.0, 0.0, -15.0, 0.0, 6.0, 15.0, 0.0, -10.0, -6.0, 6.0, 0.0, 10.0, 15.0, 0.0, -6.0, 0.0, -15.0, 0.0]
# t = [5.0, 1.0, 3.0, 2.0, 4.0, 0.5, 1.0, 2.0, 1.0, 3.0, 0.5, 1.5, 2.5, 1.0, 1.5, 1.0, 5.0, 1.0, 5.0]
