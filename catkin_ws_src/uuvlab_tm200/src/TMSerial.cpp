...
char TMSerial::Serialport_Open(std::string DEVICE_PORT)
{
    int Ret;
    Ret=serialib1->Open(DEVICE_PORT.c_str(),115200);                            // Open serial link at 115200 bauds
    if (Ret!=1) {                                                           // If an error occured...
        printf ("Error while opening port. Permission problem ? (Port: %s)\n",DEVICE_PORT.c_str());        // ... display a message ...
...
void TMSerial::On_SerialRX(uuvlab_msgs::Imu *msg_imu){
    char serialBuffer[128];
    serialib1->Read(serialBuffer,128,0);                         // Step 1: read the received data buffer of the Serial Port
    ...
    if(EP_SUCC_ == eP.On_RecvPkg(rxData, rxSize, &header)){    // Step 2: Tell the CAL that new data has arrived.
        ...
        case EP_CMD_Raw_GYRO_ACC_MAG_:{
            ...
            if(EP_SUCC_ == eOD.Read_Ep_Raw_GyroAccMag(&ep_Raw_GyroAccMag)){
                ...
                float magX = ep_Raw_GyroAccMag.mag[0];
                float magY = ep_Raw_GyroAccMag.mag[1];
                float magZ = ep_Raw_GyroAccMag.mag[2];
                ...
                msg_imu->magnetic_field.x = magX;
                msg_imu->magnetic_field.y = magY;
                msg_imu->magnetic_field.z = magZ;
        ...
        case EP_CMD_Q_S1_E_:{
            ...
            if(EP_SUCC_ == eOD.Read_Ep_Q_s1_e(&ep_Q_s1_e)){ // Step 3.3: If we decided that the received Quaternion should be used,
                ...
                yaw = std::atan2(sinr_cosp, cosr_cosp);
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle np ("~");
    ros::NodeHandle nh;
    np.param<std::string>("device_port",device_port,"/dev/ttyS0");

    tmSerial1.Serialport_Open(device_port);

    uuvlab_msgs::Imu msg_imu;
    ros::Publisher TM200_pub = nh.advertise<uuvlab_msgs::Imu>("imu", 1000);
