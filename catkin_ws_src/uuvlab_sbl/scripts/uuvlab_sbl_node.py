...
GRAVITY = 9.81  # m/s^2
ATM_PRESSURE = 101325  # Pa (average sea level atmospheric pressure)
...
def waterlinked(base_url):
    ...
    def set_depth(data,args):
        base_url = args  # Extract the base URL passed as an argument
        depth = (data.fluid_pressure - ATM_PRESSURE) / (DENSITY_WATER * GRAVITY)
        rospy.loginfo("Depth: %s",depth)
	payload = {'depth': depth, 'temp': 6.5} #Depth Fixed temp for now (degC).
	try:
    ...
    rospy.Subscriber('bluerov2/pressure',FluidPressure,set_depth,base_url)
...
if __name__ == '__main__':
    try:
	base_url = "http://192.168.3.94" #doesn't do anything, we tkae base_url from launch file
	if len(sys.argv) > 1:
